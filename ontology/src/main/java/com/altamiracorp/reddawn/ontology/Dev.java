...
public class Dev extends Base {
    ...
    protected int defineOntology(TitanGraph graph, TitanVertex entity) {
        ...
        person.setProperty(PropertyName.GLYPH_ICON.toString(), "/resource/bb94e4ffc8290bd0d07a919a010a4c5c69eb820d3ed4b6442da0fd217d74d058");
        person.setProperty(PropertyName.COLOR.toString(), palette.remove(0));
        graph.commit();
        addPropertyToConcept(graph, person, "birthDate", "birth date", PropertyType.DATE);

        TitanVertex org = getOrCreateConcept(graph, entity, ORGANIZATION_TYPE);
        org.setProperty(PropertyName.GLYPH_ICON.toString(), "/resource/8777a8592b14db5a7d4d151d9887f9500077adbfac7e30fecd987093299602da");
        org.setProperty(PropertyName.COLOR.toString(), palette.remove(0));
        graph.commit();
        addPropertyToConcept(graph, org, "formationDate", "formation date", PropertyType.DATE);

        TitanVertex company = getOrCreateConcept(graph, org, COMPANY_TYPE);
        company.setProperty(PropertyName.GLYPH_ICON.toString(), "/resource/8777a8592b14db5a7d4d151d9887f9500077adbfac7e30fecd987093299602da");
        company.setProperty(PropertyName.COLOR.toString(), palette.remove(0));
        graph.commit();
        addPropertyToConcept(graph, company, "netIncome", "net income", PropertyType.CURRENCY);

        TitanVertex location = getOrCreateConcept(graph, entity, LOCATION_TYPE);
        location.setProperty(PropertyName.GLYPH_ICON.toString(), "/resource/698ca9f70bbfce7bf7a4bdd5cdb8348d9062b31fa8085cc11b75f747e1b5c86b");
        location.setProperty(PropertyName.COLOR.toString(), palette.remove(0));
        graph.commit();
        addPropertyToConcept(graph, location, "geoLocation", "geo location", PropertyType.GEO_LOCATION);

        TitanVertex phoneNumber = getOrCreateConcept(graph, entity, PHONE_NUMBER_TYPE);
        phoneNumber.setProperty(PropertyName.GLYPH_ICON.toString(), "/resource/97979a0b19d327b519029fc0f7846a73d72961e6725d47391060fc956d4596dc");
        ...
        emailAddress.setProperty(PropertyName.GLYPH_ICON.toString(), "/resource/b2dd45fefb26bd75011fd58d22ee537b441c6e4840f5da556435227d6e191cd3");
        emailAddress.setProperty(PropertyName.COLOR.toString(), palette.remove(0));
        graph.commit();
        addPropertyToConcept(graph, emailAddress, "active", "active", PropertyType.STRING);
