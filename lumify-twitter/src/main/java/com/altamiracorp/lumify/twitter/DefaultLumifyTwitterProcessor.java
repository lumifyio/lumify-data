...
public class DefaultLumifyTwitterProcessor extends BaseArtifactProcessor implements LumifyTwitterProcessor {
    ...
    public Vertex parseTwitterUser(final String processId, final JSONObject jsonTweet, final Vertex tweetVertex) {
        ...
        userVertexMutation.setProperty(PropertyName.TITLE.toString(), screenName, visibility);
        userVertexMutation.setProperty(PropertyName.CONCEPT_TYPE.toString(), handleConcept.getId(), visibility);
        ...
        getAuditRepository().auditRelationship(AuditAction.CREATE.toString(), userVertex, tweetVertex, labelDispName, processId, "", lumifyUser);
    ...
    public void extractEntities(final String processId, final JSONObject jsonTweet, final Vertex tweetVertex,
        ...
        if (tweetText != null && !tweetText.trim().isEmpty()) {
            ...
            for (TermMentionModel mention : mentions) {
                ...
                termVertexMutation.setProperty(PropertyName.TITLE.toString(), sign, visibility);
                termVertexMutation.setProperty(PropertyName.ROW_KEY.toString(), rowKey, visibility);
                termVertexMutation.setProperty(PropertyName.CONCEPT_TYPE.toString(), concept.getId(), visibility);
                ...
                auditRepo.auditRelationship(AuditAction.CREATE.toString(), tweetVertex, termVertex, relDispName, processId, "", user);
                graph.flush();
            }
        }
    }
    ...
    public void retrieveProfileImage(final String processId, final JSONObject jsonTweet, Vertex tweeterVertex) {
        ...
        if (screenName != null && !screenName.isEmpty() && imageUrl != null && !imageUrl.isEmpty()) {
            try {
                ...
                auditRepo.auditRelationship(AuditAction.CREATE.toString(), tweeterVertex, imageVertex, labelDisplay, processId, "", user);
                ...
                imageBuilder.setProperty(PropertyName.GLYPH_ICON.toString(), String.format(GLYPH_ICON_FMT, imageVertex.getId()), visibility);
                ...
                if (!edges.hasNext()) {
                    ...
                    auditRepo.auditRelationship(AuditAction.CREATE.toString(), tweeterVertex, imageVertex, displayName, processId, "", user);
                }
                graph.flush();
