...
public class DefaultLumifyTwitterProcessor extends BaseArtifactProcessor implements LumifyTwitterProcessor {
    ...
    public Vertex parseTwitterUser(final String processId, final JSONObject jsonTweet, final Vertex tweetVertex) {
        ...
        getAuditRepository().auditRelationship(AuditAction.CREATE, userVertex, tweetVertex, labelDispName, processId, "", lumifyUser, false, lumifyVisibility.getVisibility());
    ...
    public void extractEntities(final String processId, final JSONObject jsonTweet, final Vertex tweetVertex,
        ...
        if (tweetText != null && !tweetText.trim().isEmpty()) {
            ...
            for (TermMentionModel mention : mentions) {
                ...
                auditRepo.auditRelationship(AuditAction.CREATE, tweetVertex, termVertex, relDispName, processId, "", user, false, lumifyVisibility.getVisibility());
                graph.flush();
            }
        }
    }
    ...
    public void retrieveProfileImage(final String processId, final JSONObject jsonTweet, Vertex tweeterVertex) {
        ...
        if (screenName != null && !screenName.isEmpty() && imageUrl != null && !imageUrl.isEmpty()) {
            try {
                ...
                auditRepo.auditRelationship(AuditAction.CREATE, tweeterVertex, imageVertex, labelDisplay, processId, "", user, false, lumifyVisibility.getVisibility());
                ...
                if (!edges.hasNext()) {
                    ...
                    auditRepo.auditRelationship(AuditAction.CREATE, tweeterVertex, imageVertex, displayName, processId, "", user, false, lumifyVisibility.getVisibility());
                }
                graph.flush();
