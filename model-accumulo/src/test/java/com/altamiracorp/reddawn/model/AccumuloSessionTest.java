...
public class AccumuloSessionTest {
            ...
            while (accumuloRow.hasNext()) {
                ...
                if ("testColumnFamily1".equals(columnFamilyString)) {
                    if ("1testColumn1".equals(columnNameString)) {
                        ...
                        Assert.assertEquals(111L, new com.altamiracorp.reddawn.model.Value(accumuloColumn.getValue().get()).toLong().longValue());
                    ...
                    if ("2testColumn1".equals(columnNameString)) {
                        ...
                        Assert.assertEquals(222L, new com.altamiracorp.reddawn.model.Value(accumuloColumn.getValue().get()).toLong().longValue());
                    } else {
                        fail("invalid column name: " + columnFamilyString + " - " + columnNameString);
                    }
                } else {
    ...
    public void testFindByRowKeyWithOffset() throws TableNotFoundException, MutationsRejectedException {
        ...
        List<ColumnFamily> colFams = accumuloSession.findByRowKeyWithOffset(TEST_TABLE_NAME, "testRowKey1", queryUser, 1L, 2L, "test.*");
