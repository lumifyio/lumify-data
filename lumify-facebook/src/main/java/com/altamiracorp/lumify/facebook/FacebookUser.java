...
public class FacebookUser {
    ...
    public Vertex process(JSONObject userJson, Graph graph, AuditRepository auditRepository, OntologyRepository ontologyRepository, User user, Authorizations authorizations) throws ParseException {
        ...
        if (userJson.has(EMAIL) && !userJson.getString(EMAIL).equals(JSONObject.NULL)) {
            ...
            if (queryEmailVertex == null) {
                ...
                auditRepository.auditVertexElementMutation(emailBuilder, emailVertex, PROCESS, user, lumifyVisibility.getVisibility());
            } else {
            ...
            auditRepository.auditRelationship(AuditAction.CREATE, userVertex, emailVertex, labelDisplayName, PROCESS, "", user, lumifyVisibility.getVisibility());
            graph.flush();
        }
        ...
        auditRepository.auditVertexElementMutation(userVertexMutation, userVertex, PROCESS, user, lumifyVisibility.getVisibility());
    ...
    protected void createProfilePhotoVertex(Vertex pictureVertex, Vertex userVertex, Graph graph, AuditRepository auditRepository, OntologyRepository ontologyRepository, User user, Authorizations authorizations) {
        ...
        auditRepository.auditVertexElementMutation(userVertexMutation, userVertex, PROCESS, user, lumifyVisibility.getVisibility());
        auditRepository.auditVertexElementMutation(pictureVertexMutation, pictureVertex, PROCESS, user, lumifyVisibility.getVisibility());
        userVertex = userVertexMutation.save();
        ...
        auditRepository.auditRelationship(AuditAction.CREATE, userVertex, pictureVertex, labelDisplay, PROCESS, "", user, lumifyVisibility.getVisibility());
